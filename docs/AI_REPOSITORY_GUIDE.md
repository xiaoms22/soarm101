...
│   └── lerobot/
├── record_config.yaml              # 数据采集配置文件（不上传 GitHub）
└── README.md                       # 项目说明
```
...
| Handeye 相机 | Camera 1 | 手眼相机，640×480 |
...
- **环境名称**：`lerobot-so101`
- **Python 版本**：3.14.4
- **环境路径**：`C:\Users\shuot\miniforge3\envs\lerobot-so101`

### 激活环境命令（PowerShell）

```powershell
& "C:\Users\shuot\miniforge3\shell\condabin\conda-hook.ps1"
...
位置：`D:\lerobot\soarm101\record_config.yaml`
...
- `dataset.vcodec: h264` - CPU 编码（无 NVIDIA GPU）
- `robot.disable_torque_on_disconnect: false` - 防止电机锁定

### 2. 采集命令

```powershell
python -m lerobot.scripts.lerobot_record --config_path=D:\lerobot\soarm101\record_config.yaml
```
...
```powershell
python -m lerobot.scripts.lerobot_replay --robot.type=so101_follower --robot.port=COM3 --dataset.repo_id=xiaoms22/so101-left-pilot --dataset.root=D:\lerobot\soarm101\data\so101-left-pilot --dataset.episode=0
```
...
**解决**：删除旧目录
```powershell
rm -r -Force D:/lerobot/soarm101/data/so101-left-pilot
```
...
# 检查 episode 信息
df = pd.read_parquet('data/so101-left-pilot/meta/episodes/chunk-000/file-000.parquet')
print(df[['episode_index', 'length']])

# 检查特征
with open('data/so101-left-pilot/meta/info.json', 'r') as f:
    info = json.load(f)
...
2. **record_config.yaml 不上传** - 包含本地路径
3. **每次采集前检查**：机械臂上电、相机连接、桌面布置
4. **先小批量验证** - 用 5-10 条验证协议正确性，再扩展到 33 条
5. **release 阶段最关键** - 先停稳再张爪，不要边动边开
...
3. **阅读 `docs/collect-log.md`** - 了解历史采集记录
